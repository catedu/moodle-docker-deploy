<?php
    ...
    if (isset($_POST['existe']) && $_POST['existe'] == '1') {
        $existe_user_en_Moodle = true;
    }

    $logFile = fopen("log.txt", 'a');
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: -------------------------------------------------------------------------");
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: captcha_challenge (POST): " . $_POST['captcha_challenge']);
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: captcha_text (SESSION): " . $_SESSION['captcha_text']);
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: existe (en post): " . $_POST['existe']);
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: existe_user_en_Moodle: " . $existe_user_en_Moodle);

    //Comprobamos si el captcha es correcto
    $captchaCorrecto = FALSE;

    if(isset($_POST['captcha_challenge']) && $_POST['captcha_challenge'] != '' && $_POST['captcha_challenge'] == $_SESSION['captcha_text']) {
        $captchaCorrecto = TRUE;
    }

    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: captchaCorrecto: " . $captchaCorrecto?'true':'false');
    //
    $ip = getIPAddress();
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: ip: " . $ip);
    // Listado de IPs baneadas por diversos motivos
    $ipsBaneadas = ['62.122.184.194', '85.209.11.117', '188.186.219.167', '176.111.174.153', '46.35.252.130', '154.47.21.130', '84.247.116.21', '185.203.242.202', '80.66.89.130', '46.246.122.90', '191.96.168.210', '191.96.168.161', '191.96.168.', '195.78.54.64', '84.247.116.201', '195.78.54.204', '84.247.116.174', '85.209.11.117', '85.193.65.155'];
    $ipBaneada = FALSE;
    ...
    function asignarIncidenciaA($rol, $motivo, $ciclo){
        ...
        if($motivo == "1"){//"Plataforma caída";
            return $GLOBALS["idGrupoSoporteCampus"];
        }
        ...
        elseif($motivo == "2" || $motivo == "3" ){
            switch ($ciclo) {
                ...
                case "Campus Digital FP: Administración de Sistemas Informáticos y Redes":
                    ...
                    break;
                case "Campus Digital FP: Desarrollo de Aplicaciones en Lenguaje Python":
                    return $GLOBALS["idUserCoordinacion_Campus_DALP"];
                    break;
                ...
                case "IES Sierra de Guara: Gestión Administrativa":
                    ...
                    break;
                case "IES Sierra de Guara: Ciberseguridad en Entornos de las Tecnologías de la Información":
                    return $GLOBALS["idUserCoordinacion_IES_Sierra_de_Guara_CIBER"];
                    break;
        ...
        // 6-"Cambio/Actualización de materiales";
        // 7-"Problemas con el correo @fpvirtualaragon.es";
        elseif( $motivo == "6" || $motivo == "7" ){//
            return $GLOBALS["idGrupoSoporteCampus"];
        }
    ...
    function procesaMotivo($motivo){
        switch ($motivo) {
            ...
            case "6":
                ...
                break;
            case "7":
                return "Problemas con el correo @fpvirtualaragon.es";
                break;
            default:
                return "ERROR";
        }
    ...
    function procesaPrioridad($motivo){
        switch ($motivo) {
            ...
            case "6"://Cambio/Actualización de materiales
                return "2";
                break;
            case "7"://Problemas con el correo @fpvirtualaragon.es
                return "2";
                break;
            default:
                return "ERROR";
    ...
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: parametros del form recogidos");
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: " . print_r($_POST, true));
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: captcha que se solicitaba: " . $_SESSION['captcha_text'] );
    ...
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: otros modificado: " . $otros );
    fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: otros_json: " . $otros_json );
    ...
    if(in_array($ip, $ipsBaneadas)){
        $ipBaneada = TRUE;
    }

    if($nombre_docente == "BMW"){
        $ipBaneada = TRUE;
    }

    if($accesoPermitido && $camposObligatoriosRellenos && ($captchaCorrecto || $existe_user_en_Moodle) && !$ipBaneada ){
        //////////////////////////////
        // Creo variables iniciales
        //////////////////////////////
        $date = date('d-m-Y H:i:s');        

        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: dirección IP -- " . $ip);
        ...
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: URL " . $url);

        $asignarA = asignarIncidenciaA($rol, $motivo, $ciclo);
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: incidencia asignada a " . $asignarA);
        ...
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: token -- " . $token);
        ...
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: JSON issue -- " . $jsonIssue);
        ...
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: result -- " . var_export(json_decode($result, true), true));

        if ($result === false || $result == null) {
            fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php : Curl error .. " . curl_error($curl));
        }
        curl_close($curl);
        ...
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: respuesta -- " . $respuesta );

        $incidenciaCreada = $respuesta["issue"];

        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: incidenciaCreada -- " . $incidenciaCreada );
        
        $incidenciaCreadaId = $incidenciaCreada["id"];

        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: incidenciaCreadaId -- " . $incidenciaCreadaId );
        ...
        if( $exitoCreandoIncidencia ){
            ...
            $cuerpo .= 'su incidencia realizada el ' . $date . ' ha sido recogida en nuestro sistema con el id <strong>'. $incidenciaCreadaId .'</strong>.<br/>';
            /*
            cuando en estos email meten spam y se manda por correo AST nos bloquea la cuenta por enviar emails de spam
...
<?php
    ...
    if( !$accesoPermitido ){
        ...
        $h3 = 'Su IP está baneada. No puede enviar incidencias. Contacte con el departamento técnico si cree que se trata de un error.'; 
        fwrite($logFile, "\n" . date("d/m/Y H:i:s") . " action.php: ipBaneada -- IP: '" . $ip . "'");
