...
if(($_POST)){
	...
	foreach($_POST as $nombre_campo => $valor){
		if (substr($nombre_campo,0,1)=='#'){
			...
			if (is_numeric($valor)){
				...
				$DB->execute($sql_insert_values,array('id_encuesta'=>$id_encuesta,'encuesta'=>$encuesta,'fase'=>$fasecampo,'codigo1'=>$ciclocampo,'codigo2'=>$modulocampo,'codigo3'=>$preguntacampo,'respuesta1'=>$valor,'respuesta2'=>""));
			}else{
				$valor=filter_var($valor,FILTER_SANITIZE_FULL_SPECIAL_CHARS);
				$valor=substr($valor,0,1000);
				if(!$DB->record_exists_sql($sql_check_exists,array('id_encuesta'=>$id_encuesta,'encuesta'=>$encuesta,'fase'=>$fasecampo,'codigo1'=> $ciclocampo,'codigo2'=>$modulocampo,'codigo3' => $preguntacampo)))
				$DB->execute($sql_insert_values,array('id_encuesta'=>$id_encuesta,'encuesta'=>$encuesta,'fase'=>$fasecampo,'codigo1'=>$ciclocampo,'codigo2'=>$modulocampo,'codigo3'=>$preguntacampo,'respuesta1'=>-1,'respuesta2'=>$valor));
			}
		}
	}
	...
	<center><h1 style="font-weight: bold;" >Encuesta enviada.</h1>
		<h2>Muchas gracias por tu colaboraci&oacute;n.  Tus comentarios ser&aacute;n de gran  utilidad para mejorar la calidad de la formaci&oacute;n.</h2></center>
		<hr SIZE=1 WIDTH="100%" COLOR="#c3d7f0" style="float:right; margin-left:30px;"/><br>
		<?php
	}else{
