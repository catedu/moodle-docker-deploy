...
if(isloggedin() && $DB->record_exists('role_assignments',array('roleid'=>18,'userid'=>$USER->id))){
  //es coordinador
  $sqlcentro = "SELECT course_categories.id
  FROM {course_categories} course_categories
  JOIN {course} course ON course.category = course_categories.id
  ...
  AND roleid = 18";
  $ciclo = $DB->get_field_sql($sqlcentro,array('userid'=>$USER->id),MUST_EXIST);
}else{
...
$sql_profesores_por_centro = "SELECT role_assignments.id,course.id as courseid, user.id as userid, CONCAT(user.firstname,' ',user.lastname) as name, course.fullname, course.shortname
FROM {user} user
...
$profesores = array_values($DB->get_records_sql($sql_profesores_por_centro,array('coursecategory'=>$ciclo)));
if(isset($_POST['num_prof'])){
  ...
  $mpdf->AddPage('P');
  $stylesheet = file_get_contents('../informes/alumnos.css');
  ...
  $mpdf->WriteHTML('<div style="position: absolute; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto;"><img src="../informes/portada_informe_profesores.png" width="90%" /></div>');
  $mpdf->WriteHTML("
  ...
  foreach ($profesores as $profesor) {
    $header = array (
      'odd' => array (
        ...
        'C' => array (
          'content' => substr($profesor->fullname,strrpos($profesor->fullname,'_')+1),
          'font-size' => 10,
          'font-style' => 'B',
          'font-family' => 'serif',
          'color'=>'#000000'
    ...
    $mpdf->AddPage('L');
  $dom = new DOMDocument();
  $dom->loadXML($_POST[$profesor->userid.'-calendar']);
  //quitamos los divs
...
for($i=0;$i<$nodes->length;$i++){
  ...
  $img = $_POST[$profesor->userid.'-clicks'];
  ...
  $mpdf->WriteHTML('
  <div style="width:100%;max-height:240px"><img style="max-height:240px;margin: 0 auto" src='.$img.'></div>
  ...
  <?php
  $sql_clicks_dia = "SELECT count(action) AS clicks, week(from_unixtime(timecreated),1) AS date
  ...
  for($i=0;$i<count($profesores);$i++){
    $calendario = $profesores[$i]->userid.'-calendar';
    $clicks = $profesores[$i]->userid.'-clicks';
...
<script type="text/javascript">
      google.charts.load("45", {packages:["calendar"]});
       ...
       dataTable.addRows([
         <?php
           foreach ($conexiones as $dia) {
             $dia = explode("-",$dia->dia);
             echo "[ new Date($dia[0], $dia[1]-1, $dia[2]), 10 ],";
           }
           //echo "[ new Date(2016, 9, 4), 10 ],";
          ?>
  ...
  <body>
    ...
    <form id='image' method='post'>
      ...
      <?php
        foreach ($profesores as $profesor) {
       ?>
        <div id='<?=$profesor->userid?>-calendar'></div>
        <div id='<?=$profesor->userid?>-clicks'></div>
      <?php
}
       ?>
    </form>
