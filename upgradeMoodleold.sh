...
showHelp () {
    echo 'usage: upgradeMoodle.sh [-p] [-y] [-e env-update] -u "dirsite" -d "upgrade_version_template"'
    echo "Options:"
    echo "-p -> By default, moodle-code dir is deleted. With -p preserve this directory"
...
( $YES || (read -r -p "Do you want to make UPGRADE? Its not an UPDATE...[s/N] " RESP && [[ "$RESP" =~ ^([sS]|[sS][iI]|[yY][eE][sS]|[yY])$ ]] )|| exit 0) 
