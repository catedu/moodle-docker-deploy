...
use block_xp\local\reason\chapter_read_reason;
 ...
 */
class read_chapter implements ruletype, ruletype_with_goal, ruletype_with_limit, ruletype_with_profile, with_iconography {
    ...
    public function get_display_name(): lang_string {
        return new lang_string('ruletypereadchapter', 'block_xp');
    }
    ...
    public function get_icon(): ?icon {
        global $CFG;
        if ($CFG->branch <= 401) {
            return new fa_icon('book');
        }
        return new fa_icon('book-open');
    }
    ...
    public function get_repeat_limit_options(): array {
        return [
            ...
            (new repeat_option(limit_spec::SCOPE_ENV | limit_spec::SCOPE_OBJECT))
                ->set_once_label(new lang_string('onceperchapter', 'block_xp')),
        ];
    }

    public function get_profile(): profile {
        return new cm_profile('book');
    }

    public function get_short_description(): lang_string {
        return new lang_string('ruletypereadchapterdesc', 'block_xp');
    }

    public function is_action_compatible(action $action): bool {
        return $action->get_type() === 'book_chapter_viewed';
    }
    ...
    public function make_reason(action $action): reason {
        $reason = new chapter_read_reason();
