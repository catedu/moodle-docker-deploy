...
/**
   ...
   */
let tinyFontcolorEventSource=null;const handleRow=event=>{(event.preventDefault(),tinyFontcolorEventSource&&tinyFontcolorEventSource!==event.target)||(event.target.classList.contains("del")?event.target.parentNode.parentNode.remove():event.target.classList.contains("add")&&(element=>{const currRow=element.parentNode.parentNode,newRow=currRow.cloneNode(!0),parts=newRow.querySelector("input").getAttribute("name").split("_"),num=parseInt(parts[parts.length-1]),newNum=num+1,re=new RegExp("_"+num.toString()+"$");newRow.querySelectorAll("input").forEach((input=>{["name","id","value"].forEach((attr=>{if("value"===attr)return void(input.value="");let content=input.getAttribute(attr).replace(re,"_"+newNum.toString());input.setAttribute(attr,content)}))})),currRow.parentNode.insertBefore(newRow,currRow.nextSibling);const button=currRow.querySelector("button");button.classList.remove("add"),button.classList.add("del"),button.innerHTML="-",jscolor.install(newRow),newRow.querySelector("button").addEventListener("click",(function(e){handleRow(e),tinyFontcolorEventSource=null})),newRow.querySelectorAll("input").forEach((input=>{input.addEventListener("keypress",(function(e){tinyFontcolorEventSource=e.target}))}))})(event.target))};_exports.init=name=>{const root=document.querySelector("."+name);if(!root)return;const buttons=root.getElementsByTagName("button");for(let i=0;i<buttons.length;i++)buttons.item(i).addEventListener("click",(function(e){handleRow(e),tinyFontcolorEventSource=null}));const inputs=root.getElementsByTagName("input");for(let i=0;i<inputs.length;i++)inputs.item(i).addEventListener("keypress",(function(e){tinyFontcolorEventSource=e.target}))}}));
