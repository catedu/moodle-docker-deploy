...
$sql_alumno_asignaturas = "SELECT user.id as userid,user.username, ppropio.ppropio as password, user.firstname, user.lastname, user.email,GROUP_CONCAT(DISTINCT(course.id)) as courses
FROM {user} user
...
WHERE user.firstname not like '%\_%'
...
//AND user.email='catalan1001@hotmail.com'
...
<div style="width:90%;margin-left:10%;" >
  ...
  foreach ($usuarios as $user) {
	echo "<p><i id='$user->userid' style='visibility:hidden' class='fa fa-file-text-o' aria-hidden='true'></i> <span style='display:inline-block; width: 15px;'></span>$user->firstname $user->lastname ($user->email)</p>";
    ob_end_flush();
    flush();
  }
  ?>
...
foreach ($usuarios as $user) {
  ...
  $mpdf->WriteHTML('<div style="position: absolute; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto;"><img src="portada_informe.png" width="90%" /></div>');
  $mpdf->WriteHTML("
...
if ((count($usuarios)>count($batch))&& $errors==0) { //Si quedan informes por generar, refrescamos
