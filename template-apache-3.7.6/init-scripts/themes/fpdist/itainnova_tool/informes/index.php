...
?>
<style>
	.centrosCoordinadores{
		display:flex;
		justify-content:flex-start;
		align-items: center;
		width: 1025px;
	}
	.centrosCoordinadores .col{
		width: 20%;
		display:flex;
		margin: 7px;
	}
	...
	#checkAllCenters,
	.centrosCoordinadores .col input{
		position:relative;
		margin: 7px;
		top: -3px;
	}
...
<div id="tabs" style="height:500px;margin-top:10px">
	...
	<div id="coordinadores">
		<div style="width:90%">
			<center>
				<p>Elige el centro o centros para crear los informes</p>
				
				<p><label><input type='checkbox' id='checkAllCenters'>Marcar Todos</label></p>

				<form method="get" action="coordinadores.php">
					<?php
						$sqlcentro = "SELECT id, name
										FROM {course_categories}
										WHERE parent NOT IN (SELECT id FROM {course_categories})
										AND id NOT IN (0,18,27,52,53,55,58)
										ORDER BY name";
						$centros = $DB->get_records_sql($sqlcentro);
						$ind = 0;
						echo '<div class="row centrosCoordinadores">';
						foreach ($centros as $centro) {
							echo "<div class='col'><label><input type='checkbox' class='centroCoordinador' name='centros[]' value='$centro->id'>$centro->name</label></div>";

							$ind++;
							if ($ind % 5 == 0 && $ind != count($centros)) { 
								echo '</div><div class="row centrosCoordinadores">';
							}
						}
						echo '</div>';
					?>
		...
		<center>
			<form method="get" action="profesores.php">
				...
				<div style="margin:15px auto;text-align:center;">
					<input type='checkbox' name='batch' value=1 checked>Avanzar al siguiente curso autom&aacute;ticamente
				</div>
				<input type="submit" value="Enviar informes" style="margin-top:20px">
	...
	<div id="alumnos-enviar">
		<center>
			<form action="alumnos.php" method="get">
				...
				<label>
					Número de informes por iteraci&oacute;n
					<input style="width:6ch;" name="step" type="number" placeholder="Num. informes por iteraci&oacute;n" step="5" min="5" max="100" value="25">
				</label>
...
<script>
	...
	$(function() {
		...
		$('#checkAllCenters').click(function(){
			if($(this).is(':checked')){
				$('.centroCoordinador').prop('checked', true);
			}else{
				$('.centroCoordinador').prop('checked', false);
			}
		})
	});
